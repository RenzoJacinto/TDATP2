...
amount_path = "aproximation_amount_packages.txt"
times_path = "aproximation_times.txt"
solutions_path = "aproximation_solutions.txt"
solutions_len_path = "aproximation_solutions_len.txt"
log = LogManager()

def run_aproximation_tests():
...
def test_1_package_returns_1_container_with_the_package():
    packages = [1]
    expected = [[1]]
    result = packaging_aproximation(packages)
...
def test_given_on_the_problem_statement():
    ...
    expected = [[0.4], [0.8], [0.5, 0.1], [0.7], [0.6, 0.1], [0.4, 0.2, 0.2]]
    result = packaging_aproximation(packages)
    ...
    assert result.__contains__(expected[4])
    assert result.__contains__(expected[5])
...
def test_of_volume():
    ...
    for amount_packages in volumes:
        ...
        for i in range(cant_ejecuciones):
            ...
            start_time = time()
            solution = packaging_aproximation(packages)
            end_time = time()
